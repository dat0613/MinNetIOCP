...
public:
	...
	template <typename ... args>
	void RPCudp(std::string methodName, MinNetRpcTarget target, args&&... parameters);// RPC를 호출할때

	template <typename ... args>
	void RPCudp(std::string methodName, MinNetUser * target, args&&... parameters);// RPC를 호출할때
...
inline void MinNetComponent::RPC(std::string methodName, MinNetRpcTarget target, args && ...parameters)
{
	...
	gameObject->GetNowRoom()->SendRPC(gameObject->GetID(), name, methodName, target, parametersPacket, true);
}
...
inline void MinNetComponent::RPC(std::string methodName, MinNetUser * target, args && ...parameters)
{
	...
	gameObject->GetNowRoom()->SendRPC(gameObject->GetID(), name, methodName, target, packet, true);
}
...
inline void MinNetComponent::RPCudp(std::string methodName, MinNetRpcTarget target, args && ...parameters)
{
	...
	gameObject->GetNowRoom()->SendRPC(gameObject->GetID(), name, methodName, target, parametersPacket, false);
}

template<typename ...args>
inline void MinNetComponent::RPCudp(std::string methodName, MinNetUser * target, args && ...parameters)
{
	MinNetPacket * packet = nullptr;
	...
	gameObject->GetNowRoom()->SendRPC(gameObject->GetID(), name, methodName, target, packet, false);
}
